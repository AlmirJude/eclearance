...
@endif
    ...
    @if(!$activePeriod)
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
            ...
            <p class="mt-2 text-sm text-yellow-600">There is currently no active clearance period. Please check back later.</p>
        </div>
    @elseif(!$clearanceRequest)
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
            ...
            <h3 class="mt-4 text-lg font-medium text-blue-800">Clearance Not Yet Generated</h3>
            <p class="mt-2 text-sm text-blue-600">Your clearance request for <strong>{{ $activePeriod->name }}</strong> has not been created yet. Please contact the admin.</p>
        </div>
    @else
        {{-- Period Info --}}
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">{{ $activePeriod->name }}</h3>
                    <p class="text-sm text-gray-600">{{ $activePeriod->academic_year }} - {{ $activePeriod->semester }} Semester</p>
                </div>
                <div class="text-right">
                    ...
                    @if($clearanceRequest->status === 'completed')
                        ...
                        <button onclick="printClearanceSlip()"
                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-600 text-white hover:bg-green-700 transition-colors">
                            🖨 Print Slip
                        </button>
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <div class="flex items-center justify-between mb-2">
                <h4 class="text-sm font-medium text-gray-700">Overall Progress</h4>
                <span class="text-sm font-semibold text-gray-900">{{ $progress }}%</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-4">
                <div class="h-4 rounded-full transition-all duration-500 {{ $progress == 100 ? 'bg-green-500' : 'bg-blue-500' }}" 
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h4 class="text-lg font-semibold text-gray-900">Required Signatures</h4>
                <p class="text-sm text-gray-500 mt-1">Complete requirements and get signatures from each office/organization below</p>
            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    @elseif($item['can_sign'])
                                        <p class="text-xs text-yellow-600">Awaiting signature from signatory</p>
                                    @else
                        ...
                        <div class="mt-3 ml-14 flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                @if($item['requirement_count'] === 0)
                                    <span class="text-xs text-gray-500 italic">No documents required</span>
                                @else
                                    {{-- Requirement Status --}}
                            ...
                            @if($item['status'] === 'rejected')
                                <a href="{{ route('clearance.submit-requirements', $item['id']) }}" 
                                   class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md transition-colors text-white bg-orange-600 hover:bg-orange-700">
                                    <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="mt-6 bg-gray-50 rounded-lg p-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <span class="text-gray-600">Ready for Signature</span>
                </div>
                <div class="flex items-center space-x-2">
                    <span class="w-3 h-3 rounded-full bg-gray-400"></span>
                    <span class="text-gray-600">Blocked (Waiting)</span>
                </div>
                <div class="flex items-center space-x-2">
