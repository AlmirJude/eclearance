                        ...
                        <td class="px-6 py-2">
                            @if($signatory->departments)
                                @foreach($signatory->departments as $deptId)
                                    ...
                                    @endphp
                                    @if($dept)
                                        <span class="inline-block px-2 py-1 text-xs bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 rounded mr-1 mb-1">{{ $dept->Abbreviation }}</span>
                                    @endif
                                @endforeach
                            @else
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 dark:bg-gray-900 dark:bg-opacity-75">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg">
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    ...
                    {{-- Departments --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Departments (Optional - leave empty for all departments)</label>
                        <div class="border border-gray-300 dark:border-gray-700 rounded-lg max-h-40 overflow-y-auto bg-gray-50 dark:bg-gray-900 p-2">
                            @foreach($allDepartments as $dept)
                                <label class="flex items-center gap-2 p-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded cursor-pointer">
                                    <input 
                                        type="checkbox" 
                                    ...
                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ $dept->name }} ({{ $dept->Abbreviation }})</span>
                                </label>
                            @endforeach
                        </div>
                        @error('departments') <span class="text-red-500 dark:text-red-400 text-sm">{{ $message }}</span> @enderror
                    </div>

                    {{-- Year Levels --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Year Levels (Optional - leave empty for all years)</label>
