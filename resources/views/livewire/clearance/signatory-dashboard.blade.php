...
<div class="p-6">
    <div class="mb-6">
        ...
        <flux:subheading size="lg" class="mt-1">Manage Your Clearance Signatures</flux:subheading>
    </div>
    ...
    @if(count($signatoryEntities) === 0)
        <div class="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6 text-center">
            ...
            <h3 class="mt-4 text-lg font-medium text-yellow-800 dark:text-yellow-300">No Signatory Role Assigned</h3>
            <p class="mt-2 text-sm text-yellow-600 dark:text-yellow-400">You are not currently assigned as a signatory for any organization.</p>
        </div>
    @else
        ...
        @if($selectedEntity)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <button wire:click="filterByStatus('pending')" 
                        class="bg-yellow-50 dark:bg-yellow-900/30 rounded-lg p-4 border-2 transition-all hover:shadow-md text-left
                            {{ $statusFilter === 'pending' ? 'border-yellow-500 dark:border-yellow-400 ring-2 ring-yellow-200 dark:ring-yellow-800' : 'border-yellow-200 dark:border-yellow-800' }}">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <svg class="h-8 w-8 text-yellow-400 dark:text-yellow-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <div class="ml-4">
                            <p class="text-sm font-medium text-yellow-800 dark:text-yellow-300">Pending</p>
                            <p class="text-2xl font-semibold text-yellow-900 dark:text-yellow-200">{{ $pendingCount }}</p>
                        </div>
                    </div>
                </button>
                <button wire:click="filterByStatus('approved')"
                        class="bg-green-50 dark:bg-green-900/30 rounded-lg p-4 border-2 transition-all hover:shadow-md text-left
                            {{ $statusFilter === 'approved' ? 'border-green-500 dark:border-green-400 ring-2 ring-green-200 dark:ring-green-800' : 'border-green-200 dark:border-green-800' }}">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <svg class="h-8 w-8 text-green-400 dark:text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <div class="ml-4">
                            <p class="text-sm font-medium text-green-800 dark:text-green-300">Approved</p>
                            <p class="text-2xl font-semibold text-green-900 dark:text-green-200">{{ $approvedCount }}</p>
                        </div>
                    </div>
                </button>
                <button wire:click="filterByStatus('rejected')"
                        class="bg-red-50 dark:bg-red-900/30 rounded-lg p-4 border-2 transition-all hover:shadow-md text-left
                            {{ $statusFilter === 'rejected' ? 'border-red-500 dark:border-red-400 ring-2 ring-red-200 dark:ring-red-800' : 'border-red-200 dark:border-red-800' }}">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <svg class="h-8 w-8 text-red-400 dark:text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <div class="ml-4">
                            <p class="text-sm font-medium text-red-800 dark:text-red-300">Rejected</p>
                            <p class="text-2xl font-semibold text-red-900 dark:text-red-200">{{ $rejectedCount }}</p>
                        </div>
                    </div>
                </button>
            </div>
            ...
            <div class="mb-6 flex flex-wrap gap-3">
                ...
                @if($selectedEntity && $selectedEntity['label'] === 'Club')
                    ...
                    class="inline-flex items-center px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-800 transition shadow-sm">
                ...
                @if($pendingCount > 0)
                    <button wire:click="openBulkSignModal"
                            class="inline-flex items-center px-4 py-2 bg-green-600 dark:bg-green-700 text-white rounded-lg hover:bg-green-700 dark:hover:bg-green-800 transition shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="flex flex-wrap items-center gap-3 ml-auto">
                        ...
                        <div class="relative">
                            <input type="text" wire:model.live.debounce.300ms="searchQuery"
                                ...
                                class="pl-8 pr-3 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 w-60 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400" />
                        ...
                        <select wire:model.live="requirementsFilter"
                            ...
                            <option value="all" class="dark:bg-gray-700">All Submissions</option>
                            <option value="with_submissions" class="dark:bg-gray-700">Has Attached Files</option>
                            <option value="without_submissions" class="dark:bg-gray-700">No Attachments</option>
                        </select>
                        {{-- Department Filter --}}
                        ...
                        <select wire:model.live="yearLevelFilter"
                            ...
                            @foreach($availableYearLevels as $yr)
                                <option value="{{ $yr }}" class="dark:bg-gray-700">Year {{ $yr }}</option>
                            @endforeach
                        </select>
                        {{-- Clear Filters --}}
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-900">
                                <tr>
                                    ...
                                    @else
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Signed By</th>
                                ...
                                @foreach($filteredItems as $item)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $item['student_name'] }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $item['student_id'] }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $item['department'] ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $item['year_level'] !== 'N/A' ? 'Year ' . $item['year_level'] : 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900 dark:text-gray-100">{{ $item['signed_by'] ?? 'N/A' }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-500 dark:text-gray-400">{{ $item['signed_at'] ?? 'N/A' }}</div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-gray-500 dark:text-gray-400 max-w-xs truncate" title="{{ $item['remarks'] }}">
                                                    {{ $item['remarks'] ?? '-' }}
                                                </div>
                                            </td>
                                        @endif
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
                ...
                <div class="px-6 py-4">
                    @if($signAction === 'approve')
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Are you sure you want to approve this clearance?</p>
                    @else
                ...
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-900 flex justify-end space-x-3 rounded-b-lg">
                    <flux:button wire:click="$set('showSignModal', false)" variant="ghost" class="dark:text-gray-300 dark:hover:text-white">
                        Cancel
                    </flux:button>
                    @if($signAction === 'approve')
                        <flux:button wire:click="sign" variant="primary" class="bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800">
                            Approve
                        </flux:button>
                    @else
                        <flux:button wire:click="sign" variant="primary" class="bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-800">
                            Reject
                        </flux:button>
                    @endif
                </div>
    ...
    @if($showSubmissionsModal)
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-75 flex items-center justify-center z-50">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-hidden flex flex-col">
                        ...
                        <div class="space-y-4">
                            @foreach($studentSubmissions as $submission)
                                <div class="border rounded-lg p-4 
                                    {{ $submission['status'] === 'approved' ? 'bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-800' : '' }}
                                    {{ $submission['status'] === 'rejected' ? 'bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-800' : '' }}
                                    {{ $submission['status'] === 'pending' ? 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700' : '' }}">
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <h4 class="font-medium text-gray-900 dark:text-gray-100">{{ $submission['requirement_name'] }}</h4>
                                            @if($submission['requirement_description'])
                                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $submission['requirement_description'] }}</p>
                                            @endif
                                        </div>
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            {{ $submission['status'] === 'approved' ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : '' }}
                                            {{ $submission['status'] === 'rejected' ? 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-300' : '' }}
                                            {{ $submission['status'] === 'pending' ? 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300' : '' }}">
                ...
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-900 flex justify-end rounded-b-lg">
        ...
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-75 flex items-center justify-center z-50">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full mx-4">
                ...
                <div class="px-6 py-4 space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Programs / Departments</label>
                        @if(count($availableDepartments) > 0)
                            <div class="space-y-2 max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900">
                                @foreach($availableDepartments as $dept)
                        ...
                        @else
                            <p class="text-sm text-gray-400 dark:text-gray-500 italic">No departments available.</p>
                        @endif
                    </div>
                    
                    {{-- Year Level Selection --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Year Levels</label>
                            ...
                            @foreach($availableYearLevels as $yr)
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" 
                                        ...
                                        value="{{ $yr }}"
                                        class="rounded border-gray-300 dark:border-gray-600 text-green-600 dark:text-green-400 focus:ring-green-500 dark:focus:ring-green-400 bg-white dark:bg-gray-800">
                                    <span class="text-sm text-gray-700 dark:text-gray-300">Year {{ $yr }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    {{-- Match Count --}}
                    <div class="rounded-lg p-3 {{ $bulkMatchCount > 0 ? 'bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800' : 'bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700' }}">
                        <div class="flex items-center">
                            <svg class="h-5 w-5 {{ $bulkMatchCount > 0 ? 'text-green-500 dark:text-green-400' : 'text-gray-400 dark:text-gray-500' }} mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                            <span class="text-sm font-medium {{ $bulkMatchCount > 0 ? 'text-green-800 dark:text-green-300' : 'text-gray-600 dark:text-gray-400' }}">
                    ...
                    <div>
                        ...
                        <textarea wire:model="bulkRemarks" rows="2"
                                class="w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
                                placeholder="e.g. Bulk approved for clearance period..."></textarea>
                    </div>
                </div>
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-900 flex justify-between items-center rounded-b-lg">
                    <p class="text-xs text-gray-400 dark:text-gray-500">Only students whose prerequisites are met will be signed.</p>
                    <div class="flex gap-3">
                        <flux:button wire:click="$set('showBulkSignModal', false)" variant="ghost" class="dark:text-gray-300 dark:hover:text-white">
                            Cancel
                        </flux:button>
                        <flux:button 
                            wire:click="bulkSign" 
