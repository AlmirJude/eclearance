            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($periods as $period)
                    <tr class="{{ $period->is_active ? 'bg-green-50' : '' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $period->academic_year }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $period->semester }} Semester</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                            @if(!$period->is_active)
                                <button wire:click="activate({{ $period->id }})" class="text-green-600 hover:text-green-900">Activate</button>
                            @endif
                            <button wire:click="openModal({{ $period->id }})" class="text-blue-600 hover:text-blue-900">Edit</button>
                            @if($period->is_active)
                                <button wire:click="generateRequests({{ $period->id }})" wire:confirm="This will generate clearance requests for all students. Continue?" class="text-purple-600 hover:text-purple-900">
                                    Generate Requests
                                </button>
                            @endif
                            @if(!$period->is_active)
                                <button wire:click="delete({{ $period->id }})" wire:confirm="Are you sure you want to delete this period?" class="text-red-600 hover:text-red-900">Delete</button>
                            @endif
                        </td>
                    </tr>
                @empty
    ...
    @if($showModal)
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4">
                <div class="p-6">
                    <flux:heading size="lg" class="mb-4">{{ $editingId ? 'Edit Period' : 'Create Period' }}</flux:heading>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select wire:model="semester" class="w-full border-gray-300 rounded-md shadow-sm">
